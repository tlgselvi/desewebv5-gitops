...
- **WebSocket Gözlemlenebilirliği:** MCP WebSocket katmanı için uçtan uca gözlemlenebilirlik eklendi. Bu kapsamda yeni metrikler (`mcp_websocket_active_connections`, `mcp_websocket_events_published_total`), bu metrikleri izleyen Prometheus alarmları, anlık durumu gösteren bir Grafana (`websocket-observability`) paneli ve tüm bu akışı doğrulayan mock destekli bir Playwright E2E testi geliştirildi.
