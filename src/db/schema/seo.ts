...
import { users, organizations } from './saas/core.js';

// SEO Projects - Multi-tenant support with organizationId
export const seoProjects = pgTable('seo_projects', {
  ...
  orgIdx: index('seo_projects_org_idx').on(table.organizationId),
  domainIdx: index('seo_projects_domain_idx').on(table.domain),
  ownerIdx: index('seo_projects_owner_idx').on(table.ownerId),
  orgDomainIdx: index('seo_projects_org_domain_idx').on(table.organizationId, table.domain),
}));
...
// Content Management - Multi-tenant support
export const contentTemplates = pgTable('content_templates', {
  ...
  orgIdx: index('content_templates_org_idx').on(table.organizationId),
  typeIdx: index('content_templates_type_idx').on(table.type),
  orgTypeIdx: index('content_templates_org_type_idx').on(table.organizationId, table.type),
}));
...
export const seoProjectsRelations = relations(seoProjects, ({ one, many }) => ({
  organization: one(organizations, {
    fields: [seoProjects.organizationId],
...
export const contentTemplatesRelations = relations(contentTemplates, ({ one, many }) => ({
  organization: one(organizations, {
    fields: [contentTemplates.organizationId],
    references: [organizations.id],
  }),
