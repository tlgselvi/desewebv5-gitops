...
const CommandSchema = z.object({
  command: z.enum(['deploy', 'sync', 'update', 'build', 'docura', 'seo-check', 'aiops-tune', 'observe', 'secure', 'phase-update', 'rule-verify', 'self-update', 'docura-sync']),
 ...
 */
router.get('/github-docker', asyncHandler(async (req, res) => {
 ...
 */
router.post('/aiops-tune', asyncHandler(async (req, res) => {
 ...
 */
router.post('/docura-sync', asyncHandler(async (req, res) => {
 ...
 */
router.post('/phase-update', asyncHandler(async (req, res) => {
 ...
 */
router.get('/deploy', asyncHandler(async (req, res) => {
 ...
 *                         version:
 *                           type: string
 *                           description: Version tag for GitHub and Docker (e.g., v6.7.0-rc)
 *                         image_name:
 *                           type: string
  ...
  if (req.body.workflow && Array.isArray(req.body.workflow)) {
    ...
    const WorkflowSchema = z.object({
      workflow: z.array(z.object({
        step: z.enum(['rule-verify', 'observe', 'aiops-tune', 'docura-sync', 'phase-update', 'github-tag-build', 'argocd-sync', 'observability-verify']),
    ...
    logger.info('Master Control: Command execution requested', { command, params });
...
router.post('/self-update', asyncHandler(async (req, res) => {
  ...
  logger.info('Master Control: Self-update requested', { updatesCount: updates?.length || 0 });
