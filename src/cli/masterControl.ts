...
async function appendToMasterContext(
  phase: string,
  content: string,
  options: CliOptions,
): Promise<void> {
 ...
 * Command: rule-verify
 */
async function ruleVerify(options: CliOptions): Promise<void> {
 ...
 * Command: argocd-sync
 */
async function argocdSync(options: CliOptions): Promise<void> {
  console.log("🔄 Step 3: ArgoCD Sync (Production)");

  const appName =
    options.argocd_app || process.env.ARGOCD_APP_NAME || "ea-plan-v6";
 ...
 * Command: observe
 */
async function observe(options: CliOptions): Promise<void> {
  ...
  if (options.dry_run) {
    ...
    console.log(
      `[DRY RUN] Verify: ${options.verify !== false ? "enabled" : "disabled"}`,
    );
    console.log(`[DRY RUN] Would query Prometheus and Grafana via API`);
    console.log(
      `[DRY RUN] Expected metrics: up, cpu_usage_seconds_total, http_requests_total`,
    );
    return;
  }
    ...
    if (options.verify) {
      ...
      if (!verified) {
        ...
        if (isResumeMode) {
          ...
          console.log(
            "   Note: Production observability endpoints required for full verification",
          );
          // Don't exit, allow workflow to continue
        } else {
...
async function execute(options: CliOptions): Promise<void> {
  ...
  if (isStableRelease) {
    if (isResume) {
      ...
      await appendToMasterContext(
        "RESUME",
        "🔄 Resuming deployment from Step 2 (ArgoCD Sync)",
        options,
      );
    } else {
      const executionLog =
    ...
    try {
      ...
      if (isResume) {
        ...
        console.log(
          "   Note: Set ARGOCD_SERVER_URL and ARGOCD_TOKEN for production sync",
        );
 ...
 */
async function executeFullCycle(options: CliOptions): Promise<void> {
  if (options.dry_run) {
  ...
  for (let i = 1; i < args.length; i++) {
    ...
    if (arg.startsWith("--")) {
      ...
      if (booleanFlags.includes(key)) {
        ...
        else if (key === "version") options.version = nextArg;
        else if (key === "image_name") options.image_name = nextArg;
        else if (key === "registry") options.registry = nextArg;
        else if (key === "argocd_app") options.argocd_app = nextArg;
