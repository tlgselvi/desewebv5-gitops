...
export const googleService = {
  ...
  getGoogleAuthURL(): string {
    ...
    if (!clientId || !callbackUrl) {
      ...
      throw new Error(
        "Google OAuth is not configured. Please set GOOGLE_CLIENT_ID and GOOGLE_CALLBACK_URL environment variables.",
      );
    }
   ...
   */
  async verifyGoogleToken(code: string): Promise<{
    email: string;
    name: string;
    given_name?: string;
    family_name?: string;
   ...
   */
  async handleSocialLogin(email: string, name: string): Promise<{
    user: {
      id: string;
      email: string;
      role: string;
