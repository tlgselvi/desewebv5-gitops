...
export class MasterControlService {
    ...
    async verifyEnvironment() {
        ...
        try {
            ...
            try {
                const { stdout } = await execAsync(`kubectl get namespace ${this.namespaceMonitoring}`);
            ...
            try {
                const { stdout } = await execAsync(`kubectl get serviceaccount,role,rolebinding -n ${this.namespaceMonitoring}`);
            ...
            if (config.kubernetes.argocdServer) {
                try {
                    const response = await axios.get(`${config.kubernetes.argocdServer}/api/v1/applications`, {
                        headers: {
            ...
            if (config.monitoring.prometheus) {
                try {
                    ...
                    try {
                        const { stdout } = await execAsync(`kubectl get svc prometheus -n ${this.namespaceMonitoring}`);
            ...
            if (config.monitoring.grafana) {
                try {
                    ...
                    try {
                        const { stdout } = await execAsync(`kubectl get svc grafana -n ${this.namespaceMonitoring}`);
            ...
            // Check NetworkPolicy
            try {
                const { stdout } = await execAsync(`kubectl get networkpolicy -A --no-headers`);
        ...
        try {
            ...
            try {
                ...
                if (rulesExists) {
                    ...
                    const rulesHasSecurity = rulesContent.toLowerCase().includes("security") ||
                        rulesContent.toLowerCase().includes("policy");
                    const policyHasRules = policyContent.toLowerCase().includes("rules") ||
                        policyContent.toLowerCase().includes("spec");
                ...
                try {
                    ...
                    const missingFields = requiredFields.filter((field) => !policyContent.includes(field));
            ...
            // Check audit logging
            try {
                const { stdout } = await execAsync(`kubectl get configmap audit-logging -n ${this.namespaceMonitoring} 2>&1`);
        ...
        try {
            ...
            try {
                ...
                if (runs.length > 0) {
                    ...
                    status.github.pipeline.healthy =
            ...
            else if (status.github.repo.connected ||
                status.docker.registry.accessible) {
                status.sync.status = "out-of-sync";
                status.sync.mismatches = [];
                if (!status.github.repo.connected)
                    status.sync.mismatches.push("GitHub repo not connected");
                if (!status.docker.registry.accessible)
                    status.sync.mismatches.push("Docker registry not accessible");
            }
            else {
    ...
    async collectMetrics() {
        ...
        try {
            ...
            try {
                ...
                const healthResponse = await axios.get(`${prometheusUrl}/-/healthy`, {
                    timeout: 5000,
                });
            ...
            try {
                ...
                if (grafanaHealthResponse.status === 200) {
                    ...
                    try {
                        ...
                        status.grafana.dashboards = Array.isArray(dashboardsResponse.data)
                            ? dashboardsResponse.data.length
                            : 0;
                    }
                    catch {
    ...
    async trainPredictiveModels() {
        ...
        try {
            ...
            try {
                ...
                const { stdout: jobCheck } = await execAsync(`kubectl get job aiops-training -n ${this.namespaceMonitoring} -o jsonpath='{.metadata.name}' 2>&1`);
                if (jobCheck.trim() && !jobCheck.includes("NotFound")) {
                    // Job exists, check status
                    const { stdout: jobStatus } = await execAsync(`kubectl get job aiops-training -n ${this.namespaceMonitoring} -o jsonpath='{.status.conditions[?(@.type=="Complete")].status}'`);
                    if (jobStatus.trim() === "True") {
                        ...
                        try {
                            const { stdout: metricsData } = await execAsync(`kubectl get configmap aiops-training-metrics -n ${this.namespaceMonitoring} -o jsonpath='{.data}' 2>&1`);
...
spec:
  ...
  backoffLimit: 3
`;
                        await execAsync(`echo '${trainingJobYaml.replace(/'/g, "'\"'\"'")}' | kubectl apply -f -`);
                        status.trainingStarted = new Date().toISOString();
                ...
                status.error =
                    error instanceof Error ? error.message : "Training check failed";
            }
        }
        catch (error) {
    ...
    async syncDocura() {
        ...
        try {
            ...
            try {
                const { stdout } = await execAsync(`kubectl get configmap docura-index -n ${this.namespaceMonitoring} -o jsonpath='{.metadata.name}' 2>&1`);
                if (stdout.trim() && !stdout.includes("NotFound")) {
                    result.docura.synced = true;
                    result.docura.index = stdout.trim();
                    // Try to get doc count
                    try {
                        const { stdout: docCount } = await execAsync(`kubectl get configmap docura-index -n ${this.namespaceMonitoring} -o jsonpath='{.data.doc-count}' 2>&1`);
                ...
                try {
                    const docuraImage = process.env.DOCURA_IMAGE ||
                        "ghcr.io/cptsystems/docura-builder:latest";
            ...
            // SEO Observer sync
            try {
                const { stdout: cronjobName } = await execAsync(`kubectl get cronjob seo-observer -n ${this.namespaceMonitoring} -o jsonpath='{.metadata.name}' 2>&1`);
                if (cronjobName.trim() && !cronjobName.includes("NotFound")) {
                    result.seo.synced = true;
                    result.seo.cronjob = cronjobName.trim();
                    // Get last run time
                    try {
                        const { stdout: lastRun } = await execAsync(`kubectl get job -n ${this.namespaceMonitoring} -l app=seo-observer --sort-by=.status.startTime -o jsonpath='{.items[-1].status.startTime}' 2>&1`);
                ...
                try {
                    await execAsync(`kubectl apply -f deploy/seo-observer.yaml -n ${this.namespaceMonitoring} || echo "SEO Observer apply skipped"`);
                    result.seo.synced = true;
     ...
     */
    async createGitHubTagAndBuildDocker(version, imageName, registry) {
        logger.info("Master Control: Step 2 - GitHub Tag + Docker Build started", {
            version,
            imageName,
            registry,
        });
     ...
     */
    async syncArgoCDApp(appName, timeout) {
        logger.info("Master Control: Step 3 - ArgoCD Sync started", {
            appName,
            timeout,
        });
        const status = {
        ...
        try {
            ...
            try {
                const response = await axios.get(`${config.kubernetes.argocdServer}/api/v1/applications/${appNameFinal}`, {
                    headers: {
                        Authorization: `Bearer ${config.kubernetes.argocdToken}`,
                    },
                    timeout: 5000,
                ...
                if (response.data) {
                    ...
                    status.status = response.data.status.sync.status;
                    status.health = response.data.status.health.status;
                    status.syncTime = response.data.status.sync.finishedAt;
                    // Trigger sync if out of sync
                    if (status.status === "OutOfSync") {
                        try {
                            await axios.post(`${config.kubernetes.argocdServer}/api/v1/applications/${appNameFinal}/sync`, {
                                prune: true,
                                dryRun: false,
                                strategy: {
                                    hook: {
                            ...
                            while (attempts < maxAttempts) {
                                ...
                                const statusResponse = await axios.get(`${config.kubernetes.argocdServer}/api/v1/applications/${appNameFinal}`, {
                                    headers: {
                                        Authorization: `Bearer ${config.kubernetes.argocdToken}`,
                                    },
                                });
                            ...
                            if (!status.synced) {
                                status.error =
                                    "Sync timeout - application did not sync within timeout period";
                            }
                        }
                        catch (error) {
                            logger.error("Failed to sync ArgoCD application", { error });
                            status.error =
                                error instanceof Error ? error.message : "ArgoCD sync failed";
                        }
                    }
                ...
                status.error =
                    error instanceof Error ? error.message : "ArgoCD connection failed";
            }
        }
        catch (error) {
     ...
     */
    async verifyObservability(targets, metrics) {
        logger.info("Master Control: Step 4 - Observability Verification started", {
            targets,
            metrics,
        });
        ...
        try {
            ...
            if (requestedMetrics.length > 0) {
                const availableMetricNames = metricsStatus.prometheus.queries?.map((m) => m.name ?? "") ?? [];
                availableMetrics = availableMetricNames;
                missingMetrics = requestedMetrics.filter((metricName) => !availableMetrics.some((available) => available.includes(metricName) ||
            ...
            const verificationPrometheusTarget = {
                ...
                health: metricsStatus.prometheus.connected &&
                    ...
                    ? "healthy"
                    : "unhealthy",
                verified: metricsStatus.prometheus.connected &&
                    metricsStatus.prometheus.metricsCollected > 0,
            };
            ...
            const verificationGrafanaTarget = {
    ...
    async updatePhase7() {
        ...
        try {
            ...
            try {
                ...
                const { stdout: baseOutput } = await execAsync(`kubectl apply -k ${phase7BasePath} 2>&1 || echo "Base manifests apply skipped"`);
                // Apply overlays if exists
                const overlaysPath = join(process.cwd(), "deploy", "overlays", "prod");
                if (existsSync(overlaysPath)) {
                    await execAsync(`kubectl apply -k ${overlaysPath} 2>&1 || echo "Overlays apply skipped"`);
                }
                // Count applied resources
                const appliedResources = baseOutput
                    .split("\n")
                ...
                try {
                    const { stdout: servicesJson } = await execAsync(`kubectl get deployments -n ${this.namespaceWeb} -o json 2>&1`);
                    if (servicesJson && !servicesJson.includes("NotFound")) {
                        ...
                        status.servicesDeployed = deployments.map((dep) => ({
                            ...
                            status: (dep.status?.readyReplicas ?? 0) === (dep.spec?.replicas ?? 0)
                                ? "deployed"
                                : "pending",
                ...
                try {
                    ...
                    await execAsync(`kubectl patch configmap ea-plan-phase-7 -n ${this.namespaceWeb} --type merge -p '${phaseConfigMap}' || kubectl create configmap ea-plan-phase-7 --from-literal=Phase=phase-7 --from-literal=LastUpdated=${status.lastUpdated} -n ${this.namespaceWeb}`);
                }
                catch {
    ...
    async syncAIOpsSEO() {
        ...
        try {
            // AIOps Model sync
            try {
                const { stdout } = await execAsync(`kubectl get configmap aiops-model -n ${this.namespaceMonitoring} -o jsonpath='{.metadata.name}'`);
                ...
                try {
                    await execAsync(`kubectl apply -f deploy/aiops-model.yaml -n ${this.namespaceMonitoring}`);
                    result.aiops.synced = true;
            ...
            // AIOps Tuning Job
            try {
                const { stdout } = await execAsync(`kubectl get job aiops-tuning -n ${this.namespaceMonitoring} -o jsonpath='{.metadata.name}'`);
                if (stdout.trim()) {
                ...
                try {
                    await execAsync(`kubectl create job aiops-tuning --image=python:3.11-slim -n ${this.namespaceMonitoring} -- /bin/sh -c "echo 'AIOps tuning'" --dry-run=client -o yaml | kubectl apply -f -`);
                    result.aiops.tuning = true;
                }
                catch {
            ...
            // SEO Observer
            try {
                const { stdout } = await execAsync(`kubectl get cronjob seo-observer -n ${this.namespaceMonitoring} -o jsonpath='{.metadata.name}'`);
                ...
                // Try to apply SEO Observer
                try {
                    await execAsync(`kubectl apply -f deploy/seo-observer.yaml -n ${this.namespaceMonitoring}`);
                    result.seo.synced = true;
                    logger.info("SEO Observer applied");
                }
                catch {
    ...
    async manageDeploymentCycle() {
        ...
        try {
            ...
            try {
                const { stdout } = await execAsync(`kubectl get deployments -n ${this.namespaceWeb} -o json`);
            ...
            try {
                ...
                await execAsync(`kubectl patch configmap ea-plan-v6-5 -n ${this.namespaceWeb} --type merge -p '${patchJson}' || kubectl create configmap ea-plan-v6-5 --from-literal=Phase=${status.phase} --from-literal=LastUpdated=${status.lastUpdated} -n ${this.namespaceWeb}`);
            }
            catch {
                // Ignore ConfigMap update errors
            }
     ...
     */
    async generateCEOReport(command) {
        ...
        if (deploymentStatus.healthScore === 100 && rulesStatus.compliant) {
            cevap =
                "✅ Sistem tam uyumlu ve çalışır durumda. Tüm servisler aktif, kurallara uyumlu.";
        }
        else if (deploymentStatus.healthScore >= 80 &&
            rulesStatus.violations.length === 0) {
            cevap =
                "⚠️ Sistem büyük ölçüde sağlıklı, küçük iyileştirmeler öneriliyor.";
        }
        else {
        ...
        const kanit = {
            ...
            metrics: {
                ...
                complianceScore: rulesStatus.compliant
                    ? 100
                    : Math.max(0, 100 - rulesStatus.violations.length * 20),
     ...
     */
    async performSelfUpdate(updates) {
        ...
        try {
            ...
            if (updates && updates.length > 0) {
                for (const update of updates) {
                    ...
                    switch (update.type) {
                        ...
                        case "securityPolicy":
                            try {
                                ...
                                logger.error("Failed to apply securityPolicy update", {
                                    error,
                                });
                            }
                            break;
                        ...
                        case "pipeline":
                            ...
                            try {
                                ...
                                logger.error("Failed to apply pipeline version update", {
                                    error,
                                });
                            }
                            break;
                    ...
                    status.updates.push({
                        type: update.type,
                        priority: update.type === "securityPolicy" || update.type === "rules"
                            ? "high"
                            : "medium",
            ...
            if (gitToken && status.updates.some((u) => u.applied)) {
                try {
                    ...
                    await execAsync(`git commit -m "EA Plan v6.6 Self-Update: ${status.updates
                        .filter((u) => u.applied)
                        .map((u) => u.type)
                        .join(", ")}" || true`);
    ...
    async executeWorkflow(request) {
        ...
        try {
                    ...
                    switch (step.step) {
                        case "rule-verify": {
                            ...
                            if (!compliance.compliant && stepResult.status === "completed") {
                                logger.warn("Rules compliance check failed", {
                        ...
                        case "observe": {
                            ...
                            const sanitizedRequestedMetrics = Array.isArray(step.metrics)
                                ? step.metrics.filter((metric) => typeof metric === "string" && metric.length > 0)
                                : [];
                            // Filter metrics if specified
                            if (sanitizedRequestedMetrics.length > 0) {
                                const requestedMetricSet = new Set(sanitizedRequestedMetrics);
                                const filteredQueries = (metricsStatus.prometheus.queries ?? []).filter((query) => {
                                    const metricParts = query.name.split("(");
                                    const normalizedMetric = metricParts.length > 0
                                        ? metricParts[0] ?? query.name
                                        : query.name;
                                    return (requestedMetricSet.has(query.name) ||
                            ...
                            if (request.observability_check || request.full_cycle) {
                                ...
                                const availableMetrics = (metricsStatus.prometheus.queries ?? [])
                                    .map((metric) => typeof metric.name === "string" ? metric.name : "")
                                ...
                                if (sanitizedRequestedMetrics.length > 0) {
                                    ...
                                    for (const metric of availableMetrics) {
                                        if (metric) {
                                            ...
                                            const normalizedAvailable = metricParts.length > 0
                                    ...
                                    const missingMetrics = sanitizedRequestedMetrics.filter((metricName) => {
                                        const metricParts = metricName.split("(");
                                        const normalizedMetric = metricParts.length > 0
                                            ? metricParts[0] ?? metricName
                                            : metricName;
                                        return (!availableMetricSet.has(metricName) &&
                                            !availableMetricSet.has(normalizedMetric));
                                    });
                                    observabilityChecks.metrics.missing = missingMetrics;
                                    if (missingMetrics.length > 0) {
                                        logger.warn("Some requested metrics are missing", {
                                            missingMetrics,
                                        });
                                    }
                                }
                                observabilityChecks.overall = {
                                    verified: (observabilityChecks.health.prometheus ?? "healthy") ===
                                        "healthy" &&
                        ...
                        case "aiops-tune": {
                            ...
                            if (step.parameters) {
                                try {
                                    ...
                                    await execAsync(`kubectl create configmap aiops-training-params --from-literal=parameters='${paramsJson}' -n ${this.namespaceMonitoring} --dry-run=client -o yaml | kubectl apply -f -`);
                                }
                                catch {
                            ...
                            trainingStatus.modelType = modelType;
                        ...
                        case "phase-update": {
                            ...
                            if (step.apply_manifests !== false) {
                                ...
                                if (!phaseStatus.servicesDeployed ||
                                    phaseStatus.servicesDeployed.length === 0) {
                                ...
                                if (step.commit_changes && request.auto_commit) {
                                    try {
                                        const gitToken = process.env.GITHUB_SERVICE_TOKEN ||
                        ...
                        case "github-tag-build": {
                            ...
                            if (!buildStatus.tagCreated || !buildStatus.dockerImageBuilt) {
                                stepResult.status = "failed";
                                stepResult.error =
                                    buildStatus.error || "GitHub tag or Docker build failed";
                            }
                            break;
                        }
                        case "argocd-sync": {
                        ...
                        case "observability-verify": {
                            ...
                            if (!verificationStatus.verified) {
                                stepResult.status = "failed";
                                stepResult.error =
                                    verificationStatus.issues?.join(", ") ||
            ...
            if (phaseUpdateStep?.result) {
                const phaseResult = phaseUpdateStep.result;
            ...
            if (argocdSyncStep?.result) {
                const syncResult = argocdSyncStep.result;
                summary.status =
                    syncResult.status?.toLowerCase() ||
                        (syncResult.synced ? "synced" : "outofsync");
            }
            else {
                ...
                if (request.full_cycle) {
                    try {
                        ...
                        logger.warn("Failed to get deployment status for summary", {
                            error,
                        });
                    }
                }
            ...
            if (tagBuildStep?.result) {
                const buildResult = tagBuildStep.result;
                if (buildResult.tag) {
            ...
            if (phaseUpdateStep?.result) {
                ...
                if (phaseResult.servicesDeployed &&
                    phaseResult.servicesDeployed.length > 0) {
            ...
            if (request.self_update) {
                const selfUpdateExecuted = request.auto_commit ||
                    result.steps.some((s) => s.step === "self-update");
            ...
            if (!summary.healthScore)
                summary.healthScore = result.success ? 100 : 0;
            if (!summary.status)
                summary.status = result.success ? "completed" : "failed";
            if (!summary.tag &&
        ...
        switch (command) {
            ...
            case "self-update": {
                ...
                const normalizedUpdates = Array.isArray(rawUpdates)
                    ...
                    : undefined;
                await this.performSelfUpdate(normalizedUpdates && normalizedUpdates.length > 0
                    ? normalizedUpdates
                    : undefined);
                break;
            }
