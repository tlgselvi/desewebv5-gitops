import { WebSocketServer, WebSocket } from "../ws"; // Bu satırın "ws" olarak kalması kritik
 ...
 */
function handleTopicSubscription(client, message) {
    ...
    const clientId = Array.from(clients.entries()).find(([_, c]) => c === client)?.[0];
    if (clientId) {
 ...
 */
function handleTopicUnsubscription(client, message) {
    if (!client.isAuthenticated) {
        sendToClient(client, {
            type: "error",
            error: "Authentication required",
 ...
 */
const isGatewayInboundMessage = (value) => {
...
function handleMessage(client, data) {
    try {
        ...
        const rawRecord = typeof parsed === "object" && parsed !== null
            ? parsed
            : {};
...
function handleConnection(ws) {
    ...
    ws.on("close", () => {
        const clientId = Array.from(clients.entries()).find(([_, c]) => c === ws)?.[0];
        if (clientId) {
 ...
 */
export function initializeWebSocketGateway(httpServer) {
    if (wss) {
 ...
 */
export function getWebSocketGateway() {
    return wss;
}
/**
...
export function getGatewayStats() {
    const connectedClients = Array.from(clients.values()).filter((c) => c.readyState === WebSocket.OPEN).length;
    const authenticatedClients = Array.from(clients.values()).filter((c) => c.isAuthenticated && c.readyState === WebSocket.OPEN).length;
    const topics = topicSubscriptions.size;
    const totalSubscriptions = Array.from(topicSubscriptions.values()).reduce((sum, subscribers) => sum + subscribers.size, 0);
    return {
        connectedClients,
