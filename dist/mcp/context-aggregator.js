 ...
 */
const normalizeContextData = (input) => {
...
async function fetchModuleContext(module, query, authToken) {
    const portMap = {
        finbot: 5555,
        mubot: 5556,
        dese: 5557,
 ...
 */
function mergeContexts(contexts, strategy = "merge") {
    ...
    if (strategy === "latest") {
        ...
        const sorted = contexts.sort((a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime());
        return sorted[0]?.data ?? {};
    }
 ...
 */
export async function aggregateContext(request, authToken) {
    const { query, modules, priority, mergeStrategy = "merge" } = request;
    // Determine which modules to query
    let modulesToQuery = modules || ["finbot", "mubot", "dese", "observability"];
    ...
    const contextPromises = modulesToQuery.map((module) => fetchModuleContext(module, query, authToken));
    const contexts = (await Promise.all(contextPromises)).filter((ctx) => ctx !== null);
    if (contexts.length === 0) {
    ...
    const aggregated = {
        ...
        merged,
        priorities: contexts.reduce((acc, ctx) => {
            acc[ctx.module] = ctx.priority;
            return acc;
        }, {}),
        timestamps: contexts.reduce((acc, ctx) => {
 ...
 */
export async function getAggregatedContext(request, authToken, ttl = 60) {
 ...
 */
export function selectContextByPriority(aggregated, preferredModule) {
    ...
    const sortedModules = [...aggregated.modules].sort((a, b) => (aggregated.priorities[b] || 0) - (aggregated.priorities[a] || 0));
