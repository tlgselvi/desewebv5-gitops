 ...
 */
async function handleTransactionUpdated(eventData) {
    try {
        const transaction = JSON.parse(eventData.payload || "{}");
 ...
 */
async function handleAccountCreated(eventData) {
    try {
 ...
 */
async function handleBudgetUpdated(eventData) {
    try {
 ...
 */
async function sendToDLQ(messageId, eventData, error) {
    try {
        await redis.xadd(DLQ_KEY, "*", "originalMessageId", messageId, "eventType", eventData.eventType || "unknown", "payload", eventData.payload || JSON.stringify(eventData), "error", error.message, "errorStack", error.stack || "", "timestamp", Date.now().toString(), "retryCount", (parseInt(eventData.retryCount || "0") + 1).toString());
 ...
 */
async function processEvent(messageId, eventData) {
    ...
    try {
        ...
        if (retryCount >= maxRetries) {
            await sendToDLQ(messageId, { ...eventData, retryCount: retryCount.toString() }, error instanceof Error ? error : new Error(String(error)));
            ...
            await redis.xadd(STREAM_KEY, "*", ...Object.entries(retryData).flat());
...
async function processPendingMessages() {
    try {
        ...
        const pendingMessages = await redis.xpending(STREAM_KEY, CONSUMER_GROUP, "-", "+", 10, CONSUMER_NAME);
        ...
        for (const rawPending of pendingMessages) {
            ...
            const idleTime = Number.parseInt(typeof idleRaw === "string" ? idleRaw : String(idleRaw ?? "0"), 10);
            // If message has been idle for more than 5 minutes, claim it
            if (idleTime > 5 * 60 * 1000) {
                try {
                    const claimed = await redis.xclaim(STREAM_KEY, CONSUMER_GROUP, CONSUMER_NAME, 5 * 60 * 1000, messageId);
...
async function readAndProcessMessages() {
    try {
        // Read new messages
        const messages = await redis.xreadgroup("GROUP", CONSUMER_GROUP, CONSUMER_NAME, "COUNT", "10", "BLOCK", "1000", "STREAMS", STREAM_KEY, ">");
 ...
 */
export async function startFinBotConsumer() {
    if (isRunning) {
 ...
 */
export async function stopFinBotConsumer() {
    if (!isRunning) {
...
//# sourceMappingURL=finbot-consumer.js.map